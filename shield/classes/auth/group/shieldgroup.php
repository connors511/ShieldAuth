...
class Auth_Group_ShieldGroup extends \Auth_Group_Driver
{
	...
	{
		static::$_valid_groups = array_keys(\Arr::assoc_to_keyval(Model_Group::find('all'), 'id', 'name'));
	}

	protected $config = array(
		'drivers' => array('acl' => array('ShieldAcl'))
	);
	...
	public function get_name($group = null)
	{
		...
		return $group == null ?: array_map(function($v) { return $v->name; }, Model_Group::find($group));
	}
	...
	public function get_roles($group = null)
	{
		...
		return array_unique(array_map(function($v) { return $v->roles; }, Model_Group::find($group)));
	}
}

/* end of file shieldgroup.php */